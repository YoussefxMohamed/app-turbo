...
setInterval(() => {
  http.get(`http://turbo-bot-finsal-3.youssefxmohamed.repl.co`);
}, 280000);
...
client.on("message", (message) => {
  if (message.content.startsWith("كس")) {
    message.delete()
if (message.member.hasPermission("ADMINISTRATOR")){
message.delete()
       message.channel.send(
...
if (command === "embed") {
    if (!BESHO[1])
    message.delete()
      return message.channel.send(`> ** اكتبلك انا ؟**`)
      .catch(console.error);
message.delete()
...
client.on('ready', () => {
    setInterval(function () {
        let client2 = client.channels.get("ايدى الشنل")
        if (client2) {
...
/// welcome
/*const guild = client.guilds.get("872242482442809476")///ايدي السيرفر حقك
*/
client.on('guildMemberAdd', async (member) => {
...
const wel = client.channels.get("872242482753183803")/// ايدي روم الترحيب 
